...
export default function CoachFlagsScreen() {
  ...
  const flagsQuery = useQuery<FlagWithDetails[]>({
    queryKey: ["coach-pending-flags", teamIds],
    ...
    queryFn: async () => {
      ...
      for (const flag of flags) {
        ...
        const message = msg as
          | Pick<Message, "content" | "senderId" | "conversationId">
          | null
  ...
  const handleAction = (flagId: string, action: FlagAction) => {
    Alert.alert("Confirm", `Mark this flag as ${action}?`, [
      ...
      {
        text: "Confirm",
        onPress: () => actionMutation.mutate({ flagId, action }),
      },
    ])
  }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <Text style={styles.heading}>Team Flags</Text>

        {flags.length === 0 && (
          <Text style={styles.empty}>No pending flags for your teams.</Text>
        )}
